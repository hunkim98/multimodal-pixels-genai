...
import { PenTool, Coord, PanZoom, ButtonDirection, MouseMode } from "../../utils/types";
import { TouchyEvent, addEvent, removeEvent, touchy } from "@/utils/touch";
import { calculateNewPanZoomFromPinchZoom, getIsPointInsideRegion, getMouseCartCoord, getPointFromTouchyEvent, returnScrollOffsetFromMouseOffset } from "@/utils/position";
import { convertCartesianToScreen, diffPoints, getScreenPoint } from "@/utils/math";
...
export class Canvas extends EventDispatcher {
    ...
    constructor(element: HTMLCanvasElement, backgroundElement: HTMLCanvasElement, canvasWidth: number, canvasHeight: number) {
        super();
        ...
        this.setPanZoom({
            offset: {
                x: this.panZoom.scale * -this.canvasWidth / 2,
                y: this.panZoom.scale * -this.canvasHeight / 2
            }
        })
        this.initialize();
    }
    ...
    setWidth(width: number, devicePixelRatio?: number) {
        ...
        this.backgroundElement.width = devicePixelRatio ? width * devicePixelRatio : width;
    ...
    setHeight(height: number, devicePixelRatio?: number) {
        ...
        this.backgroundElement.height = devicePixelRatio ? height * devicePixelRatio : height;
    ...
    onMouseDown(evt: TouchyEvent) {
        ...
        const isPointInsideCanvas = getIsPointInsideRegion(
            mouseCartCoord,
            {
                startWorldPos: {
                    x: 0,
    ...
    render() {
        ...
        for (let i = 0; i < this.brushPoints.length; i++) {
            ...
            for (let j = 0; j < brushStroke.points.length; j++) {
                ...
                const correctedScreenPoint = getScreenPoint(convertedScreenPoint, this.panZoom);
                if (j === 0) {
